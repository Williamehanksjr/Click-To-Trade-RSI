...
class TickerWithRSIPlot:
    ...
    def update_data_and_redraw(self):
        ...
        # Robustly extract last_price (handle scalar, Series, ndarray)
        try:
            ...
            arr = np.asarray(val)
            if arr.size == 0:
                raise ValueError("No price data available to determine last_price")
            last_price = float(arr.ravel()[-1])

        # Optional debug prints (uncomment while troubleshooting)
        # print("DEBUG: df.columns=", df.columns)
        # print("DEBUG: type(p)=", type(p), "p.shape=", getattr(p, "shape", None))
        # print("DEBUG: p.tail()=", p.tail())
...
def on_click(self, event):
    # Debug: print the event fields so we can see what the backend emits
    print("DEBUG on_click: button=", getattr(event, "button", None),
          "inaxes=", getattr(event, "inaxes", None),
          "xdata=", getattr(event, "xdata", None),
          "ydata=", getattr(event, "ydata", None),
          "guiEvent=", getattr(event, "guiEvent", None))

    # Accept left clicks or when backend doesn't set button (some mac trackpad/OS combos)
